class Wicket < ActiveRecord::Base
  ...
  def attended_tickets
    tickets.where(:status_ticket_id => StatusTicket.attended.id).order("tickets.updated_at DESC")
  end

  def called_tickets
    tickets.where(:status_ticket_id => StatusTicket.called.id).order("tickets.updated_at DESC")
  end

  def pending_tickets
    tickets.where(:status_ticket_id => StatusTicket.pending.id).order("tickets.updated_at DESC")
  end
