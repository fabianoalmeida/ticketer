...
pdf.fill_color "000000"
pdf.text "#{ t('application.reports.attendances_by_days_per_wicket.name') }", :size => 20, :align => :center

pdf.move_down 20

pdf.fill_color "000000"

data = [ [ t('application.reports.attendances_by_days_per_wicket.date').to_s, t('activerecord.models.wicket').to_s,  t('application.reports.attendances_by_days_per_wicket.quantity').to_s ] ]
...
@attendances_per_wicket.each do |call_history|
  ...
  call_history[1].values[1].each_with_index do |val, index|
    ...
    data << [ index == 0 ? "#{l(current_date, :format => :default)} (#{l(current_date, :format => '%A')})" : "", call_history[1][:wickets][index].to_s, val.to_s ]
    total += val.to_i
  end
end
