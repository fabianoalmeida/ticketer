...
pdf.fill_color "000000"
pdf.text "#{ t('application.reports.attendances_by_month_per_wicket.name') }", :size => 20, :align => :center

pdf.move_down 20

pdf.fill_color "000000"

data = [ [ t('activerecord.models.wicket').to_s, t('application.reports.attendances_by_month_per_wicket.month_year').to_s,  t('application.reports.attendances_by_month_per_wicket.quantity').to_s ] ]
...
@attendances_per_month.each do |call_history|
  ...
  call_history[1].values[0].each_with_index do |val, index|
    data << [ index == 0 ? unabbreviated_month_year(call_history[0].to_s) : "", 
	call_history[1][:dates][index], val.to_s ]
    total += val
  end
end
