...
pdf.fill_color "000000"
pdf.text "#{ t('application.reports.waiting_time_by_client_per_month.name') }", :size => 20, :align => :center

pdf.move_down 20

pdf.fill_color "000000"

data = [["#{ t('application.reports.waiting_time_by_client_per_month.month_year') }", 
		 "#{ t('application.reports.waiting_time_by_client_per_month.time') }", 
		 "#{ t('application.reports.waiting_time_by_client_per_month.quantity') }"] ]
...
@waiting_time_clients_per_month.each do |call_history|
    data << [ "#{unabbreviated_month_year(call_history[:data])}", 
