...
pdf.fill_color "000000"
pdf.text "#{ t('application.reports.waiting_time_by_day_per_wicket.name') }", :size => 20, :align => :center

pdf.move_down 20

pdf.fill_color "000000"

data = [[ "#{ t('application.reports.waiting_time_by_wicket.date') }", 
          "#{ t('activerecord.models.wicket') }",
...
@waiting_time_by_wicket.each do |call_history|
  ...
  call_history[1].each_with_index do |val, index|
    ...
    data << [  index == 0 ? "#{l(current_date, :format => :default)} (#{l(current_date, :format => '%A')})" : "", 
