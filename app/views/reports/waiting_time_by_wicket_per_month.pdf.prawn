...
pdf.fill_color "000000"
pdf.text "#{ t('application.reports.waiting_time_by_wicket_per_month.name') }", :size => 20, :align => :center
...
data = [["#{ t('activerecord.models.wicket') }",
 			"#{ t('application.reports.waiting_time_by_wicket_per_month.month_year') }", 
			"#{t('application.reports.waiting_time_by_wicket_per_month.time')}",
			"#{ t('application.reports.waiting_time_by_wicket_per_month.total') }"] ]
...
@wickets_per_month.each do |call_history|
  ...
  call_history[1].each_with_index do |val, index|
    data << [  index == 0 ? call_history[0].to_s : "", 
			 unabbreviated_month_year(call_history[1][index][:data]), 
