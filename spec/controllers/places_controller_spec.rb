...
describe PlacesController do
  ...
  def mock_place(stubs={})
    @mock_place ||= mock_model(Place, stubs.merge(:local_id => "1")).as_null_object
  end
  ...
  describe "GET index" do
    it "assigns all places as @places" do
      Status.stub_chain(:active, :id){1}
      Place.stub(:where) { [mock_place] }
      get :index
  ...
  describe "DELETE destroy" do
    it "destroys the requested place" do
      Place.stub(:find).with("37") { mock_place }
      mock_place.should_receive(:status=)
